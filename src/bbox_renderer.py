...
def _detect_scale(boxes: list[tuple[int, int, int, int]], img_w: int, img_h: int) -> tuple[float, float]:
    ...
    The system prompt mandates Qwen-VL 0-1000 normalized coords for every event.
    Normalized [0, 1] is also supported for robustness against alt models.
    Raw-pixel output is no longer assumed — it caused silent mis-rendering when
    the image dimensions happened to match the coord-space ceiling.
    """
    if not boxes:
        return 1.0, 1.0
